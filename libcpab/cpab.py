...
class Cpab(object):
    ...
    def sample_transformation_with_prior(self, n_sample=1, mean=None, 
        ...
        dist = self.backend.to(dist, device=self.device)
        ...
        cov_init = self.backend.to(cov_init, device=self.device)
        ...
        for i in range(self.params.nC):
            for j in range(self.params.nC):
                ...
                block = self.backend.to(block, device=self.device)
